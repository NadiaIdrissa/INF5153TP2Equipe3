...
@Component
public class CreerLogement implements Commande<StringBuilder, String> {

    private FormulairesLogement formulairesLogement;

    @Autowired
    public CreerLogement(FormulairesLogement formulairesLogement) {
    ...
    public String execute(List<Permission> permissions, Scanner scanner, StringBuilder JWT) {
        return formulairesLogement.creerLogement(permissions, scanner, JWT);
